...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Restore session from sessionStorage if student browsed away
    try {
      const stored = sessionStorage.getItem('usv_session');
      if (stored) {
        ...
        if (data.loggedIn) {
          ...
          if (data.psContext) setPsContext(data.psContext);
          if (data.yearData) setYearData(data.yearData);
          if (data.selectedYear) setSelectedYear(data.selectedYear);
          if (data.activeTab) setActiveTab(data.activeTab);
          if (data.simulatedGrades) setSimulatedGrades(data.simulatedGrades);
          setLoggedIn(true);
        }
      }
    } catch (err) {
    ...
    if (savedUser && savedRemember && !sessionStorage.getItem('usv_session')) {
      setUserid(savedUser);
      setRememberMe(true);
    }
  ...
  // Save session to sessionStorage when relevant state changes
  useEffect(() => {
    if (loggedIn) {
      try {
        const sessionData = {
        ...
        sessionStorage.setItem('usv_session', JSON.stringify(sessionData));
      } catch (err) {
        console.error('Failed to save session to sessionStorage:', err);
      }
    } else {
      try {
        sessionStorage.removeItem('usv_session');
      } catch (err) {
        console.error('Failed to clear session from sessionStorage:', err);
      }
    }
  }, [loggedIn, userid, result, grades, studentName, academicYear, psContext, yearData, selectedYear, activeTab, simulatedGrades]);
          ...
          {isInitializing ? (
            ...
            <div className="login-section">
              ...
              <div className="login-shell">
                ...
                {/* Timetable Promo Card */}
                <a href="/orar" className="login-extra-card">
                  <div className="login-extra-icon-wrapper">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
                      <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                      <line x1="16" y1="2" x2="16" y2="6" />
                      <line x1="8" y1="2" x2="8" y2="6" />
                  ...
                  <div className="login-extra-content">
                    <h2>{tLogin('timetableCardTitle')}</h2>
                    <p>{tLogin('timetableCardDesc')}</p>
                  </div>
                  <div className="login-extra-action">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
